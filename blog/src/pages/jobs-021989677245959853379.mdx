...
**Data Flow:**
1. User sends message → Django receives
2. Current agent processes (Brainstorm → Plan → CodeGen sequence)
...
- Messages show agent name badge ("🤔 Brainstorming", "📋 Planning", "⚡ Coding")
...
### Key UX Flows
1. **New project**: Landing page → "Start New Project" → `/project/{uuid}`
2. **Code generated**: Sandpack fades in, shows live preview
3. **Refinement**: User types "add dark mode" → agent updates files → Sandpack reloads
4. **Export**: Click button → modal shows GitHub repo created → link to view
...
**Edge Cases:**
- Invalid project URL → 404 page with "Start New Project"
- Large files → Limit to 50 files max, 10KB each
- GitHub errors → Show friendly message, log for debugging
...
✅ **Multi-agent AI architecture** - LangChain agents with specialized roles
✅ **LLM integration** - Anthropic Claude API usage
✅ **Full-stack skills** - Python/Django backend + React/TypeScript frontend
✅ **Real-time features** - SSE streaming, live preview
✅ **External API integration** - GitHub repo creation
✅ **Database design** - PostgreSQL with JSONB, proper relationships
✅ **Modern tooling** - Docker, Sandpack, Vite
✅ **User experience** - Chat-based interface, instant feedback
