
## Introduction

```
As a Fullstack/Typescript/NodeJs Developer/Solution engineer, Strong in NodeJS, ReactJS, MongoDB, PostgreSQL, AWS.
```

## Website Builder

### Clothing Store

```
As a Fullstack/Typescript/NodeJs Developer/Solution engineer, Strong in NodeJS, ReactJS, MongoDB, PostgreSQL, AWS.

Making themes in Evanto/ThemeForest

Tech Stack: Vercel, Typescript, NextJS, PostgreSQL, Tailwind CSS
```

### Marking for web3 product

```

```

### Hotel app booking

```

```

### Online courses for certificates

```

```

## Upwork job analysis

```
As a Fullstack/Typescript/NodeJs Developer/Solution engineer, Strong in NodeJS, ReactJS, MongoDB, PostgreSQL, AWS.

- Analyze for me this requirement, provide me solution (simple)
- Provide me code for this solution, just core code
- Estimate the task as a freelancer (hourly rate)
- Use English

## Requirement

- Suggest me some solution sync file from web to filesystem
- The problem that in the web like feature chat app or upload files, user use it many time, idonnt wanna zip files and checksum, I can create simple electron app for writing files, but don't wanna too comlicated

```

## Claude


## Research

Give me simple code around 20 lines, using llamaindex resolve it.

Parse event storming flows → nodes/edges → store in graph DB (Neo4j).

## Documents 


```
As a Solution Engineer. Based on the code Minimum viable product / Proof of concept / Demo.
Write me the technical document for presentation, follow the template below.
Each section should have from 2-3 sentences. Simple and easy to understand (English). Technical should be camelCase and in code block.
Return document in markdown block. No need AI start/end content.

As a Solution Engineer. Based on your understand about WebRTC and LiveKit.
Write me the comparison technical document, follow the template below.
Each section should have from 2-3 sentences. Simple and easy to understand (English). Technical should be camelCase and in code block.
Return document in markdown block. No need AI start/end content.

---
title: ...
description: ...
---

# **Implement ....**
## **Overview: Purpose and Problem Solving**
- ...
## **Proposal**
- ...
## **Weighting / Pros and Cons**
### **Pros**
- ...
### **Cons**
- ...
## **Implementation Steps**
### **1. ...
- ...
### **2. ....
- ...
...
## **Conclusion**
```
